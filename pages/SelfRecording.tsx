...
import { BufferIndicator } from '../components/recording/BufferIndicator';
...
export const SelfRecording: React.FC = () => {
  ...
  const [ringBufferEnabled, setRingBufferEnabled] = useState(true); // NEW: Ring buffer toggle
  ...
  // Ring Buffer Hook (NEW)
  const ringBuffer = useRingBuffer(30); // 30 seconds buffer
    ...
    try {
      ...
      if (ringBufferEnabled && previewStream) {
        try {
          ...
          // Continue with normal recording even if buffer fails
        }
      }
  ...
  const handleRetroactiveHighlight = async () => {
    ...
    try {
      ...
      if (highlightBlob) {
        ...
        const chunkId = Date.now(); // Unique ID based on timestamp
